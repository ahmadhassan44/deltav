...
> **Positioning update (2026-09):** DeltaV is no longer framed as a Final Year Project / POC studio. Do not reintroduce FYP, academic-year, student, or "no staffing cost" language. Scope is software/compute/AI only — no hardware delivery for now. Section 3 ("What we build", 2026-09-26) lists only what we deliver: six items with small green line icons, matching the services and demos. Sections below that still describe the old POC/FYP model are superseded by what's live in `src/index.html`.
>
