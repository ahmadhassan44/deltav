...
> **Positioning update (2026-09):** DeltaV is no longer framed as a Final Year Project / POC studio. Do not reintroduce FYP, academic-year, student, or "no staffing cost" language. Scope is software/compute/AI only — no hardware delivery for now. Section 3 ("What we build", 2026-09-26) is a tile grid: icon + 2–4 plain words per tile, no descriptions, each tile linking to its demo or service page. The audience is often older, non-technical operators on manual workflows — plain words, no jargon. FAQs everywhere are `<details class="qa">` dropdowns inside `.qa-list` (CSS in both `site.css` and the homepage `<style>`; the insights and problems scripts emit the same markup). Sections below that still describe the old POC/FYP model are superseded by what's live in `src/index.html`.
>
...
>
> **Booking (2026-09-26):** Every call button ("Book a call", "Book a scoping call", "Book 30 min", "Talk through your workflow", the intake's "Schedule a call") goes straight to `https://cal.com/deltav.support/30min` in the same tab, with `data-book` on the link. Never point a call button at `#intake` or scroll the page first.
>
...
| ~~`[CALENDLY_LINK]`~~ | Replaced by cal.com (see Booking below) |
...
- Homepage hooks it relies on (the build fails if one goes missing): the `?p=` legacy shim at the top of `<body>` (the one inline script before content; `{{problems}}` is filled with live slugs), `<section id="hero">`, `#intake`, the hidden `field-subject` / `source_problem` / `source_variant` / `field-company` / `field-tried` inputs, and `window.DV_BOOK` in the "Schedule a call" handler (carries the UTM tags). The build reads the booking URL from the first `https://cal.com/…` link in `src/index.html`.
- `/work/` is the prototype gallery: `src/work/index.html` is a template (`{{prototype_count}}`, `{{prototypes}}`) that `problems.mjs` renders; the build skips the raw file. Cards link to `/<slug>/?demo=1&utm_…`; `?demo=1` opens the live prototype on arrival and incoming `utm_*` tags replace the cold-email defaults on the booking link.
- `src/lead-attribution.js` (deferred, every page) keeps the visit's first touch (entry page, channel, referrer, UTM) in sessionStorage, fills the matching hidden fields in the intake form, exposes `DV_BOOK_FROM_SITE`, and adds those UTM tags to every `a[data-book]` (skipped on problem pages, which set `DV_BOOK` with cold-email tags). Nothing is sent anywhere except with a form submission.
- `?c=` puts the company in the headline, form (`source_company`, subject) and booking-link `utm_term`; text only, 40 chars max.
- SEO (2026-09-26): `/<slug>/` is an indexable industry landing page. It has its own title, description and canonical, the problem headline as its `<h1>` (the homepage h1 drops to h2), a visible FAQ with FAQPage schema and a related guide link, and it is listed in `sitemap.xml`. All of that comes from the required `seo` block in the JSON (`title` ≤60, `description` ≤160, `faq`, optional `guide`). `/<slug>/stack/` and `?c=` links stay `noindex` with the canonical on `/<slug>/`. None go in `llms.txt`.
